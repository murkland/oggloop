...
func Load(in io.ReadSeekCloser) (beep.StreamCloser, beep.Format, Info, error) {
	...
	return &streamerAndCloser{beep.Seq(beep.Take(int(info.LoopStart), stream), beep.Loop(-1, &interval{stream, int(info.LoopStart), int(info.LoopStart + info.LoopLength)})), stream}, format, info, nil
}
